...
class tPF():
        ...
        while not rospy.is_shutdown():
            ...
            if self.oMap.started and self.tMap.started and self.anti_oMap.started and self.anti_tMap.started:
                ...
                if self.N_eff < 0.001 and self.resample_counter>10:
                    ...
                    self.resample_counter = 0
                    self.gibbs_resampling(iter_n=1)
    ...
    def initialize_PF( self , angles = np.linspace(0 , 360 ,30 ) , xRange = np.linspace(-15 , 15 , 30) , yRange = np.linspace(-15 , 15 ,30) ,fire_up = 6000, Np = 1000):
    ...
    def gibbs_resampling(self, iter_n = 1):
        self.itr = 0
        ...
        for iter in range(iter_n):
            ...
            for i, idx in enumerate(index):
                self.Rot[i].x = Rot_arr[idx].x# + np.random.normal(0,0.05)
                self.Rot[i].y = Rot_arr[idx].y# + np.random.normal(0,0.05)
            ...
            for i, idx in enumerate(index):
                self.Rot[i].theta = Rot_arr[idx].theta 
