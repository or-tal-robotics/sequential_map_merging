...
data_de = pd.read_csv("csv/MonteCarloStatistics_de_2.csv")
...
plt.figure(1)
plt.plot(avg_de, color = 'r', label = "DENDT (average)", linewidth = 1.5)
plt.plot(avg_pf, color = 'b', label = "PF (average)" , linewidth = 1.5)
plt.plot(best_run_de_overall, color = 'r', label = "DENDT (best one)", ls='-.')
plt.plot(best_run_pf_overall, color = 'b', label = "PF (best one)", ls='-.')
plt.legend()
plt.xlabel("k")
plt.ylabel("MSE")
